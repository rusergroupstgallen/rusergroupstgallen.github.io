...
(b) To preserve the year variable, we use transmutate. Of course it is perfectly fine to use `select()` first and then `mutate`. In order to achieve the rounding to one decimal, we pass the population values to `round()` with a pipe operator. Pipe in a pipe!
```{r}
gapminder %>% 
