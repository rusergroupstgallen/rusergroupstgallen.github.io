...
.center[
```{r, warning=FALSE, echo = F, out.width="100%"}
out_plot
```
]
---
...
#>>>>>>> db0532604a9dd2fd77f9a6393e73a38795fb05e5
...
.center[Tip: the function **`sapply(tibble, function)`** applies the **function** to all vectors/columns in the **tibble** and returns the result as a vector (also think about `NA` again!)]
...
```{r}
mean_male <- sapply(data_male, mean, na.rm = T)
mean_female <- sapply(na.omit(data_female), mean)
