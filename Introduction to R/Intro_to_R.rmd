...
However, they can only contain data of one type, so you cannot put a number and a string into the same vector!
...
third_vector <- c("hello", 100, "test", FALSE)
...
a Tibble is a two dimensional frame which is made of of different colums whereby each column is a distinct vector.

```{r}
tibble_test <- tibble(x = first_vector, y = second_vector)
print(tibble_test)
```
